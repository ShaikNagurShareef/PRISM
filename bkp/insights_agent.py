...
from sqlalchemy.engine import URL # --- FIX: Import URL object for safe connection strings ---
...
class DBHandlerService:
    def __init__(self, db_config: Dict[str, Any]):
        ...
        try:
            ### --- FIX: Use URL.create for robust connection string generation --- ###
            if db_type == "mysql":
                conn_url = URL.create(
...
# ==============================================================================
class BaseAgent:
    def __init__(self, agent_name: str, client: openai.OpenAI):
...
# ==============================================================================
# 5. LANGGRAPH WORKFLOW: DATA INSIGHTS
# ==============================================================================
...
def sql_generate_node(state: DataInsightsState):
    ...
    logger.info(f"[{state['session_id']}] Generated SQL: {query}")
...
def should_proceed(state: DataInsightsState):
    ...
    if state['router_decision'].get('requires_sql'):
        return 'sql_generate'
    if state['router_decision'].get('requires_plot'):
...
# ==============================================================================
app = FastAPI(title="PRISM Backend", description="Data Insights Agent Backend")
...
async def invoke_insights_workflow(request: InsightsRequest):
    try:
        ...
        initial_state = DataInsightsState(
            session_id=request.session_id,
            question=request.question,
            history=request.history,
            data_source_config=request.source_config,
            data_context=request.data_context,
            summary=request.summary,
            router_decision={}, sql_query=None, query_result=None,
            plot_code=None, plot_path=None, final_insight="", error=None,
            step_log=[]
        )
        ...
        return {
            "final_insight": final_state.get("final_insight", ""),
