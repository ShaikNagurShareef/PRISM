...
class Settings(BaseSettings):
    ...
    MAX_CORRECTION_ATTEMPTS: int = 2 # New: Set a limit for self-correction
...
class CodeExecutorService:
    ...
    def run_ml_pipeline_code(self, code: str, dataset_path: Path, session_id: str) -> Dict[str, Any]:
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            try:
                ...
                process = subprocess.run(command, cwd=temp_dir, capture_output=True, text=True, timeout=600)
                log_output = f"--- STDOUT ---\n{process.stdout}\n\n--- STDERR ---\n{process.stderr}"
                ...
                return {"success": False, "log": f"An unexpected error occurred: {e}", "artifacts": {}}
...
class ModelingPlannerAgent(BaseAgent):
    def plan(self, task: str, data_context: str, source_type: str) -> Dict[str, Any]:
...
class ModelingCodeGenAgent(BaseAgent):
    def generate(self, plan: str) -> str:
        code = self.invoke(plan, llm_type="adv")
        return code.strip().replace("```python", "").replace("```", "")

class ModelingCorrectorAgent(BaseAgent): # New Agent
...
class PredictiveModelingState(TypedDict):
    ...
    error: Optional[str]
    step_log: List[str]
    correction_attempts: int # New state field
...
def prepare_data_node(state: PredictiveModelingState):
    ...
    if config['type'] != 'file':
        state['step_log'].append("...Source is a database. Executing SQL query to create dataset.")
        sql_query = state['ml_plan'].get('data_sql_query')
        if not sql_query: return {"error": "Planner Agent failed to generate the required SQL query."}
        try:
...
def plan_pipeline_node(state: PredictiveModelingState):
    ...
    plan = modeling_planner_agent.plan(state['task_description'], state['data_context'], source_type)
    return {"ml_plan": plan, "step_log": state['step_log']}

def generate_code_node(state: PredictiveModelingState):
    state['step_log'].append("✍️ Generating ML pipeline code...")
    code = modeling_code_gen_agent.generate(state['ml_plan']['plan'])
...
def execute_code_node(state: PredictiveModelingState):
    ...
    result = code_executor_service.run_ml_pipeline_code(state['generated_code'], Path(state['dataset_path']), state['session_id'])
...
def correct_code_node(state: PredictiveModelingState): # New node
...
def check_execution_result(state: PredictiveModelingState): # New conditional edge
...
def build_predictive_modeling_graph():
    ...
    workflow.add_node("correct_code", correct_code_node) # New
    
    workflow.set_entry_point("plan_pipeline")
    workflow.add_edge("plan_pipeline", "prepare_data")
    workflow.add_edge("prepare_data", "generate_code")
    
    # This is the entry point for the execution phase
...
modeling_corrector_agent = ModelingCorrectorAgent("modeling_corrector_agent", openai_client) # New
...
async def start_modeling_pipeline(request: StartModelingRequest):
    try:
        thread_config = {"configurable": {"thread_id": request.session_id}}
        initial_state = PredictiveModelingState(session_id=request.session_id, task_description=request.task_description, source_config=request.source_config, data_context=request.data_context, step_log=[], dataset_path="", ml_plan={}, generated_code="", execution_log="", artifacts={}, error=None, correction_attempts=0)
        ...
        return {"generated_code": code, "step_log": final_state.get("step_log", []), "download_path": str(zip_path)}
    except Exception as e:
...
async def execute_modeling_pipeline(request: ExecuteModelingRequest):
    try:
        ...
        # Simply invoke with None to resume the interrupted workflow
        final_state = predictive_modeling_graph.invoke(None, thread_config)
        
        if final_state.get("error"):
            # The final error is now the one that couldn't be corrected
