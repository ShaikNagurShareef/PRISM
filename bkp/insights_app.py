...
def reset_session():
    ...
    st.session_state.sources = {} # {name: {config, summary, context, messages}}
...
# --- Main App Logic ---
st.set_page_config(page_title="PRISM Data Insights", layout="wide")
st.title("🔮 PRISM: Data Insights Agent")
...
with st.sidebar:
    ...
    with st.expander("➕ Add New Data Source", expanded=True):
        ...
        with db_tab:
            # --- CORRECTED: Selectbox is now OUTSIDE the form for reactivity ---
            ...
            with st.form("db_form"):
                ...
                # --- CORRECTED: Form content is now conditional on the selectbox above ---
                if db_type == "sqlite":
