...
# ==============================================================================
# --- Voice-to-Text Dependencies and Helper ---
# ==============================================================================
try:
    import speech_recognition as sr
except ImportError:
    st.error("Speech recognition library not found. Please install the required libraries using:\n\n`pip install SpeechRecognition PyAudio`")
    st.stop()
...
def voice_to_text():
    ...
    try:
        ...
        st.error(f"An error occurred with the microphone: {e}")
        return None
...
def get_capabilities():
    try:
        ...
        st.error(f"Error fetching AutoRAG capabilities: {e}. Please ensure the AutoRAG backend is running on port 8001.")
        return None
...
def reset_session():
    ...
    st.session_state.modeling = {"source_name": None, "task": "", "generated_code": None, "step_log": [], "execution_results": None}
...
def render_management_page():
    ...
    with st.container(border=True):
        ...
        if 'rag_create_name' not in st.session_state: st.session_state.rag_create_name = ""
        if 'rag_create_desc' not in st.session_state: st.session_state.rag_create_desc = ""
        if 'rag_create_prompt' not in st.session_state: st.session_state.rag_create_prompt = "You are a helpful AI assistant..."

        # CORRECTED PATTERN: Handle button action and rerun before drawing the widget
        st.markdown("RAG Name*")
        name_cols = st.columns([10, 1])
        with name_cols[1]:
            if st.button("🎤", key="mic_rag_create_name", help="Use voice to input name"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.rag_create_name = transcript
                    st.rerun()
        with name_cols[0]:
            st.text_input("name", key="rag_create_name", placeholder="e.g., Financial Analyst Bot", label_visibility="collapsed")

        st.markdown("Description*")
        desc_cols = st.columns([10, 1])
        with desc_cols[1]:
            if st.button("🎤", key="mic_rag_create_desc", help="Use voice to input description"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.rag_create_desc = transcript
                    st.rerun()
        with desc_cols[0]:
            st.text_area("description", key="rag_create_desc", placeholder="A short summary of what this RAG is for.", label_visibility="collapsed")

        st.markdown("Custom System Prompt*")
        prompt_cols = st.columns([10, 1])
        with prompt_cols[1]:
            if st.button("🎤", key="mic_rag_create_prompt", help="Use voice to input system prompt"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.rag_create_prompt = transcript
                    st.rerun()
        with prompt_cols[0]:
            st.text_area("system_prompt", key="rag_create_prompt", height=150, label_visibility="collapsed")

        with st.form("create_rag_form"):
            submitted = st.form_submit_button("Create RAG Instance", use_container_width=True, type="primary")
            if submitted:
                final_name, final_desc, final_prompt = st.session_state.rag_create_name, st.session_state.rag_create_desc, st.session_state.rag_create_prompt
                if not all([final_name.strip(), final_desc.strip(), final_prompt.strip()]):
                    st.warning("RAG Name, Description, and System Prompt are all required fields.")
                else:
                    payload = {"name": final_name, "description": final_desc, "system_prompt": final_prompt, "config": {"llm_provider": llm_provider, "llm_model": llm_model, "embedding_provider": embed_provider, "embedding_model": embed_model, "vector_store": vector_store, "chunker": chunker_name}}
                    with st.spinner("Creating RAG instance..."):
                        result = create_rag(payload)
                        if result: 
                            st.success(f"Successfully created RAG: {result['name']}")
                            st.session_state.rag_create_name, st.session_state.rag_create_desc, st.session_state.rag_create_prompt = "", "", "You are a helpful AI assistant..."
...
def render_edit_page():
    ...
    if st.session_state.get('editing_rag_id_loaded') != rag_id:
        ...
        st.session_state.edit_rag_name = rag_details.get('name', '')
        st.session_state.edit_rag_desc = rag_details.get('description', '')
        st.session_state.edit_rag_prompt = rag_details.get('system_prompt', '')
        st.session_state.editing_rag_id_loaded = rag_id
    
    if st.button("← Back to Management"): 
        st.session_state.editing_rag_id = None
        st.session_state.editing_rag_id_loaded = None 
        st.rerun()
    ...
    with st.container(border=True):
        ...
        rag_details = next((rag for rag in get_all_rags() if rag['id'] == rag_id), {})
        current_config = rag_details.get('config', {})
        ...
        name_cols = st.columns([10, 1])
        with name_cols[1]:
            if st.button("🎤", key="mic_rag_edit_name", help="Use voice to input name"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.edit_rag_name = transcript
                    st.rerun()
        with name_cols[0]:
            st.text_input("name", key="edit_rag_name", label_visibility="collapsed")

        st.markdown("Description*")
        desc_cols = st.columns([10, 1])
        with desc_cols[1]:
            if st.button("🎤", key="mic_rag_edit_desc", help="Use voice to input description"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.edit_rag_desc = transcript
                    st.rerun()
        with desc_cols[0]:
            st.text_area("description", key="edit_rag_desc", label_visibility="collapsed")

        st.markdown("Custom System Prompt*")
        prompt_cols = st.columns([10, 1])
        with prompt_cols[1]:
            if st.button("🎤", key="mic_rag_edit_prompt", help="Use voice to input system prompt"):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.edit_rag_prompt = transcript
                    st.rerun()
        with prompt_cols[0]:
            st.text_area("system_prompt", key="edit_rag_prompt", height=150, label_visibility="collapsed")

        with st.form("edit_rag_form"):
            submitted = st.form_submit_button("Save Changes", use_container_width=True, type="primary")
            if submitted:
                final_name, final_desc, final_prompt = st.session_state.edit_rag_name, st.session_state.edit_rag_desc, st.session_state.edit_rag_prompt
                if not all([final_name.strip(), final_desc.strip(), final_prompt.strip()]):
...
def render_chat_page():
    ...
    if rag_id not in st.session_state.chat_histories or not st.session_state.chat_histories[rag_id]:
        with st.spinner("Loading chat history..."):
            ...
            reconstructed_history = [{"role": msg["role"], "content": msg["content"], "sources": msg.get("metadata", {}).get("sources", [])} for msg in history]
    ...
    def submit_autorag_prompt():
        if st.session_state.autorag_user_input:
            st.session_state.autorag_prompt_to_process = st.session_state.autorag_user_input
            st.session_state.autorag_user_input = ""

    if 'autorag_user_input' not in st.session_state:
        st.session_state.autorag_user_input = ""

    input_cols = st.columns([1, 10, 1])
    with input_cols[0]:
        if st.button("🎤", key="mic_autorag_chat", help="Ask with your voice", use_container_width=True):
            transcript = voice_to_text()
            if transcript:
                st.session_state.autorag_user_input = transcript
                st.rerun()
    with input_cols[1]:
        st.text_input("Ask a question...", key="autorag_user_input", label_visibility="collapsed", on_change=submit_autorag_prompt)
    with input_cols[2]:
        st.button("➤", key="send_autorag_chat", help="Send message", use_container_width=True, on_click=submit_autorag_prompt)

    if "autorag_prompt_to_process" in st.session_state and st.session_state.autorag_prompt_to_process:
        prompt = st.session_state.autorag_prompt_to_process
        del st.session_state.autorag_prompt_to_process
...
logo_base64 = get_base64_of_image("PRISM.png")
        ...
        with db_tab:
            ...
            with st.form("db_form"):
                ...
                if db_type == "sqlite":
                    ...
                    default_dbs = {"postgresql": "mydatabase", "mysql": "mydatabase"}
                    ...
                    db_config = {"type": db_type, "host": host, "port": port, "username": username, "password": password, "database": database}
                    ...
                    with st.spinner(f"Analyzing {name}..."):
                        try:
                            ...
                            if result and "summary" in result:
                                st.session_state.sources[name].update({"analyzed": True, "summary": result["summary"], "data_context": result["data_context"], "messages": [{"role": "assistant", "content": result["summary"]}]})
        ...
        if st.form_submit_button("Resume Session"):
            if resume_id:
                ...
                if loaded_state:
                    ...
                    st.success("PRISM session resumed successfully!"); st.rerun()
                else:
...
with insights_tab:
    if not st.session_state.active_source_name:
        ...
        def submit_insights_prompt():
            if st.session_state.insights_user_input:
                st.session_state.insights_prompt_to_process = st.session_state.insights_user_input
                st.session_state.insights_user_input = ""

        if 'insights_user_input' not in st.session_state:
            st.session_state.insights_user_input = ""

        input_cols = st.columns([1, 10, 1])
        with input_cols[0]:
            if st.button("🎤", key="mic_insights_chat", help="Ask with your voice", use_container_width=True):
                transcript = voice_to_text()
                if transcript:
                    st.session_state.insights_user_input = transcript
                    st.rerun()
        with input_cols[1]:
            st.text_input(f"Ask a question...", key="insights_user_input", label_visibility="collapsed", on_change=submit_insights_prompt)
        with input_cols[2]:
            st.button("➤", key="send_insights_chat", help="Send message", use_container_width=True, on_click=submit_insights_prompt)

        if "insights_prompt_to_process" in st.session_state and st.session_state.insights_prompt_to_process:
            prompt = st.session_state.insights_prompt_to_process
            del st.session_state.insights_prompt_to_process
        ...
        if active_source["messages"] and active_source["messages"][-1]["role"] == "user":
            with st.chat_message("assistant"):
                with st.spinner("PRISM is thinking..."):
                    try:
                        # ... (backend call logic is unchanged) ...
                        history_str = "\n".join([f"{m['role']}: {m['content']}" for m in active_source["messages"][:-1]])
                        payload = {"session_id": st.session_state.session_id, "question": active_source["messages"][-1]["content"], "history": history_str, "source_config": active_source["config"], "data_context": active_source["data_context"], "summary": active_source["summary"]}
...
with modeling_tab:
    ...
    with st.container(border=True):
        ...
        if not source_options:
            ...
            if 'modeling_task_desc' not in st.session_state:
                st.session_state.modeling_task_desc = ""
            
            st.markdown("Describe your modeling objective")
            
            # --- CORRECTED PATTERN: Handle button action and rerun before drawing the widget ---
            desc_cols = st.columns([10, 1])
            with desc_cols[1]:
                if st.button("🎤", key="mic_modeling_desc", help="Describe your objective with voice"):
                    transcript = voice_to_text()
                    if transcript:
                        st.session_state.modeling_task_desc = transcript
                        st.rerun() # Immediately rerun
            with desc_cols[0]:
                task_description = st.text_area(
                    "task_description", key="modeling_task_desc",
                    placeholder="e.g., Predict the 'price' column...", label_visibility="collapsed"
                )
            
            if st.button("Generate ML Pipeline", type="primary", disabled=not selected_source or not task_description):
                # ... (rest of modeling tab logic is unchanged) ...
                st.session_state.modeling = {"source_name": selected_source, "task": task_description, "generated_code": None, "step_log": [], "execution_results": None}
                source_info = st.session_state.sources[selected_source]
                payload = {"session_id": st.session_state.session_id, "task_description": task_description, "source_config": source_info["config"], "data_context": source_info["data_context"]}
    ...
    if st.session_state.modeling.get("generated_code"):
        # ... (rest of modeling tab is unchanged) ...
        with st.container(border=True):
            ...
            with col1:
                ...
                if download_path and Path(download_path).exists():
                    with open(download_path, "rb") as fp:
                        st.download_button(label="Download Project as .zip", data=fp, file_name=f"{st.session_state.session_id}_pipeline.zip", mime="application/zip", use_container_width=True)
            with col2:
...
with autorag_tab:
    ...
    st.info("This feature connects to a separate AutoRAG backend. Please ensure it is running on port 8001.")
...
if st.session_state.get("sources"):
    state_to_save = {"sources": st.session_state.sources, "active_source_name": st.session_state.active_source_name, "modeling": st.session_state.modeling}
